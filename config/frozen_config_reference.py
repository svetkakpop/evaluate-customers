"""Residual sequence model — GPU edition.

Upload next to this script:  train.parquet, gate_pack.npz, oof_residuals.npz

WHY THIS AND NOT ANOTHER LTV MODEL
----------------------------------
The LTV-trained GPU network is better solo than the small CPU one (1.6755 vs
1.6821) and yet LESS orthogonal to the stack (corr_orth 0.0167 vs 0.0249). That is
the measured failure mode: the better a sequence model gets at the LTV target, the
more it converges on what the tabular stack already knows, and the ensemble pays
only for what is new.

So the target here is not the LTV. It is the part of the LTV that the stack gets
wrong, out-of-fold:

    r(u,a) = z_true(u,a) - z_gbdt_oof(u,a)

Folds are purged by 42 days. A single gbdt stands in for the whole stack because
on the holdout it correlates 0.99740 with it and their residuals correlate 0.99771,
and fold 2 reproduces the known holdout gbdt to 1e-4.

Anchor 378 is held out: it has an OOF residual, but training on it would destroy
the only honest yardstick we have.

The output is NOT meant to be added as z_stack + lambda*r by hand. It ships as one
more direction and the analytic stacker finds the weight, which is strictly better.
"""
import json
import math
import os
...
WINDOW = 364
HORIZON = 30
CHANNELS = 96             # smaller than the LTV net: the residual is mostly noise
BLOCKS = 7
DROPOUT = 0.2
WD = 3e-4
BATCH = 768
...
gp = np.load("gate_pack.npz")
oof = np.load("oof_residuals.npz")
MU_TRUE = float(gp["mu_true"])
DAY0 = int(gp["day0"])
FINAL_ANCHOR = int(gp["final_anchor"])
OOF_ANCHORS = [int(a) for a in oof["anchors"]]
OOF_R = oof["residuals"]                    # (n_anchors, n_users)
OOF_W = oof["weights"]
HOLD_ANCHOR = OOF_ANCHORS[-1]
DEV_ANCHORS = OOF_ANCHORS[:-1]
print(f"train anchors {DEV_ANCHORS}\nholdout {HOLD_ANCHOR}")

PARQUET_BYTES = 180_148_911
if os.path.getsize("train.parquet") != PARQUET_BYTES:
    raise SystemExit("train.parquet is truncated — re-copy it from Drive")
...
R, BUY, BUY7, POS, WT = {}, {}, {}, {}, {}
for i, a in enumerate(OOF_ANCHORS):
...
assert np.allclose(np.log1p((cum_gmv[:, HOLD_ANCHOR + HORIZON + 1] - cum_gmv[:, HOLD_ANCHOR + 1])),
                   gp["y_hold_log"], atol=1e-3), "holdout target mismatch"
print("holdout target matches the CPU pipeline")
del cum_gmv, cum_buy
...
# ----------------------------------------------------------------- gate pieces
T = gp["T"].astype(np.float64)
S = gp["d_stack_hold"].astype(np.float64)


def marginal(dnew):
    """<r,T>^2/<r,r> per sample, which equals rho_orth^2 * V_T — the most the
    ensemble can gain from this direction. Both forms are scale-invariant."""
    d = dnew - dnew.mean()
    r = d - S * (d @ S) / (S @ S)
    rho = float(np.corrcoef(r, T)[0, 1])
    A = np.stack([S, d])
    V = A @ A.T / len(T)
    C = A @ T / len(T)
    a = np.linalg.pinv(V) @ C
    s1 = float(np.sqrt(np.mean(T ** 2) - a @ C))
    s0 = float(np.sqrt(np.mean(T ** 2) - (S @ T / len(T)) ** 2 / (S @ S / len(T))))
    return dict(rho=rho, dmse=rho * rho * float(np.mean(T ** 2)), gain=s0 - s1, joint=s1)
...
for ep in range(start_ep, EPOCHS):
    ...
    g = marginal(zh)
    print(f"epoch {ep}  rho_orth={g['rho']:+.4f}  dMSE={g['dmse']:.5f}  "
          f"gain={g['gain']:+.6f}  joint={g['joint']:.6f}  ({time.time()-t0:.0f}s)", flush=True)
    np.save(f"hold_resid_ep{ep}.npy", zh)
    np.save(f"pred_resid_ep{ep}.npy", predict(FINAL_ANCHOR))
    torch.save({"model": model.state_dict(), "opt": opt.state_dict(), "sched": sched.state_dict(),
                "scaler": scaler.state_dict(), "epoch": ep}, CKPT)

# ----------------------------------------------------------------- pick and ship
best_ep, best_g = None, None
for ep in range(EPOCHS):
    f = f"hold_resid_ep{ep}.npy"
    if not os.path.exists(f):
        continue
    g = marginal(np.load(f))
    print(f"  epoch {ep}: rho_orth={g['rho']:+.4f} dMSE={g['dmse']:.5f} joint={g['joint']:.6f}")
    if best_g is None or g["dmse"] > best_g["dmse"]:
        best_ep, best_g = ep, g
print(f"\nbest for the ENSEMBLE is epoch {best_ep}: {best_g}")
print("  (solo quality is deliberately not the criterion here)")

pred = np.load(f"pred_resid_ep{best_ep}.npy")
plog = pred - pred.mean() + MU_TRUE
out = np.clip(np.expm1(plog), 0, None)
pd.DataFrame({"user_id": gp["user_id"], "predict": out}).sort_values("user_id").to_csv(
    "submission_resid.csv", index=False)
np.save("hold_resid.npy", np.load(f"hold_resid_ep{best_ep}.npy"))
np.save("pred_resid.npy", pred)
print("wrote submission_resid.csv, hold_resid.npy, pred_resid.npy")
print(f"needed rho_orth to beat the leader: 0.0505   this: {best_g['rho']:.4f}")
